extends GutTest
## Phase 15/16: pack_ability_flags/unpack_ability_flags round-trip, now
## covering all 7 flags (attack, skillshot, ability_q/e/r/f, Phase 16's
## boot_active) -- not unit-tested before Phase 15, added alongside the
## R/F extension since a bit-order mistake here would silently swap
## which ability a remote peer's input actually re-triggers server-side.


func test_pack_and_unpack_round_trips_every_flag_independently() -> void:
	for flag_index in range(7):
		...
		assert_eq(
			restored.boot_active_pressed, sample.boot_active_pressed, "flag_index %d" % flag_index
		)
