...
7. Main Menu + offline character select (no duplicate-class check) +
   ...
   for headless testing). **Not started** — scope designed via `/think`
   2026-09-03, see Slice 7 below.
...
### Slices 7-10 (Lobby / Character-Select / Room-Config / Perks / LAN Discovery) — designed via `/think` 2026-09-03, NOT STARTED

Full design lives in this session's `/think` transcript; the durable
summary below is what the `ship-phase` skill (`.claude/skills/ship-
phase/SKILL.md`) reads before re-deriving each slice's plan from
scratch. Confirmed by the human owner:
...
- `MatchState.Phase` simplifies from `{LOBBY, CHARACTER_SELECT,
  LOADING, IN_PROGRESS, POST_GAME}` to `{LOBBY, LOADING, IN_PROGRESS,
  POST_GAME}` — `LOBBY` now means "connected, in Room Config."
  `CHARACTER_SELECT` is dropped (genuinely unused: character pick is
  local/pre-connection now). **`LOADING` is kept, reconsidered
  2026-09-03** — not for asset-streaming UI (this project has none),
  but as a real sync barrier: this codebase already hit the exact bug
  class it prevents once before (`net/dev_bootstrap.gd`'s own comment
  documents `PlayerSpawner._ready()` racing `host()/join()` and
  producing an "authority RPC not allowed" engine error). The Room
  Config → in-game scene change has the identical risk across peers,
  now that it's a real `change_scene_to_file` transition instead of
  today's single main-scene load: each peer changes scene locally,
  reports "loaded" to the server via RPC once its own `TestArena.tscn`
  (and therefore `PlayerSpawner`) exists, and the server only calls
  `enter_in_progress()` once every currently-connected peer has
  reported in — same "wait for everyone" bookkeeping shape Room
  Config's own ready-check (Slice 8) already needs, reused rather than
  invented twice. No progress bar, no percentage, no asset list — just
  the barrier.
